...
│   ├── __init__.py
│   ├── service.py             #   OpenAI Assistants API integration
...
- One OpenAI Assistant created with custom golf coaching instructions.
...
4. Single API call to OpenAI: "Generate exactly 5 improvement insights as a JSON array."
5. Response cached in `insights_cache` table.
6. Landing page reads from cache — no API call on page load.
...
- [ ] Add `openai_api_key` and `openai_assistant_id` to `config.py`
- [ ] Create `scripts/create_assistant.py` — one-time setup script to create the OpenAI Assistant
- [ ] Create `insights/serializers.py` — round data → structured prompt context
- [ ] Create `insights/prompts.py` — system instructions and query templates
- [ ] Create `insights/service.py` — OpenAI API call, cache read/write
...
Everything is done via code — no web UI required.
...
The assistant ID is not a secret — it's generated by the setup script and stored in `.env`.
...
4. Add it to your `.env` file:
   ...
   OPENAI_API_KEY=sk-...
   ```
5. Run the one-time assistant setup script:
   ```bash
   uv run python scripts/create_assistant.py
   ```
   This creates the Golf Improvement Coach assistant and prints the ID.
6. Add the returned ID to `.env`:
   ```
   OPENAI_ASSISTANT_ID=asst_...
   ```

To update the assistant's instructions later, run the same script with `--update`.
...
dependencies = [
    ...
    "openai>=1.30",        # Stage 5 — Assistants API
]
```
...
openai_assistant_id: str = ""                    # Stage 5
```
